...
public:
	pool(memory* allocator = nullptr, trees_type trees_type = trees_type::RB) :
		_allocator(allocator), _trees_type(trees_type)
	{
		switch (trees_type)
		{
			/*case trees_type::AVL:
				...
				break;*/
		case trees_type::RB:
			_pool = new red_black_tree<std::string, scheme, string_comparer>();
			break;
...
public:
	pool(const pool& other) :
		_allocator(other._allocator), _trees_type(other._trees_type)
	{
		switch (_trees_type)
		{
			/*case trees_type::AVL:
