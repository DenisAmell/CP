...
protected:
	struct avl_node : binary_search_tree<tkey, tvalue, tkey_comparer>::node
	{
		...
		avl_node(tkey const& new_key, tvalue&& new_value) : binary_search_tree<tkey, tvalue, tkey_comparer>::node(new_key, std::move(new_value))
		{
		}
	};
...
avl_tree<tkey, tvalue, tkey_comparer>::avl_tree(memory* allocator,
	logger* logger) : binary_search_tree<tkey, tvalue, tkey_comparer>(new avl_insertion_template_method(this),
		new binary_search_tree<tkey, tvalue, tkey_comparer>::reading_template_method(this),
	...
	std::stack<typename binary_search_tree<tkey, tvalue, tkey_comparer>::node**>& path_to_subtree_root_exclusive)
{
	...
	size_t result = _tree->balance_factor(subtree_root_address);
...
template <
	...
	typename tvalue,
	typename tkey_comparer>
void avl_tree<tkey, tvalue, tkey_comparer>::avl_insertion_template_method::constructor_node(
	typename binary_search_tree<tkey, tvalue, tkey_comparer>::node* ptr_node,
	tkey const& key,
	tvalue&& value) const
{
	new (ptr_node) typename avl_tree<tkey, tvalue, tkey_comparer>::avl_node(key, std::move(value));
}
...
{
	reinterpret_cast<typename avl_tree<tkey, tvalue, tkey_comparer>::avl_node*>(ptr_node)->height = 0;
}
...
template <
	...
	typename tvalue,
	typename tkey_comparer>
size_t avl_tree<tkey, tvalue, tkey_comparer>::avl_insertion_template_method::get_size_node()
{
	...
	typename binary_search_tree<tkey, tvalue, tkey_comparer>::node* node)
{
	...
	{
		return reinterpret_cast<avl_tree<tkey, tvalue, tkey_comparer>::avl_node*>(node)->height;
	}
}
...
void avl_tree<tkey, tvalue, tkey_comparer>::fix_height(
	typename binary_search_tree<tkey, tvalue, tkey_comparer>::node** subtree_address)
{
	size_t left_height = *subtree_address != nullptr ? get_height((*subtree_address)->left_subtree_address) : 0;
	size_t right_height = *subtree_address != nullptr ? get_height((*subtree_address)->right_subtree_address) : 0;

	if (*subtree_address != nullptr)
	{
		reinterpret_cast<avl_tree<tkey, tvalue, tkey_comparer>::avl_node*>(*subtree_address)->height = (left_height > right_height ? left_height : right_height) + 1;
	}
}
...
size_t avl_tree<tkey, tvalue, tkey_comparer>::balance_factor(
	typename binary_search_tree<tkey, tvalue, tkey_comparer>::node* subtree_address)
{
	size_t left_height = get_height(subtree_address->left_subtree_address);
	size_t right_height = get_height(subtree_address->right_subtree_address);
