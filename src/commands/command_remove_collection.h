...
public:
	command_remove_collection(char* filename = nullptr) :
...
bool command_remove_collection::can_execute(std::string const& request)
{
    ...
    ->get_logger()
    ->log("command_remove_collection::can_execute(std::string const& request) called",
...
void command_remove_collection::execute(std::string const&)
{
	if (_filename_save != nullptr)
	{
		...
		file_save.is_open();
		file_save << "REMOVE_COLLECTION " + _pool_name + "  "
			+ _scheme_name + " "
			+ _collection_name << std::endl;
	}

	logger_singleton::get_instance()
		->get_logger()
		->log("command_remove_collection::execute(std::string const&) called",
			logger::severity::trace);
