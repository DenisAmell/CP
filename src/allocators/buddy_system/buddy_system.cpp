...
void* buddy_system::get_first_avail_block() const
{
	return *get_first_avail_block_ptr();
}

void** buddy_system::get_first_avail_block_ptr() const
{
	return reinterpret_cast<void**>(reinterpret_cast<unsigned char*>(_all_memory) + get_service_part_allocator_size() - sizeof(void*));
}

void* buddy_system::get_pointer_avail_prev_block(void* target_ptr) const
{
	return *reinterpret_cast<void**>(reinterpret_cast<unsigned char*>(target_ptr) + 1);
}

void* buddy_system::get_pointer_avail_next_block(void* target_ptr) const
{
	return *reinterpret_cast<void**>(reinterpret_cast<unsigned char*>(target_ptr) + 1 + sizeof(void*));
}

size_t buddy_system::get_service_part_allocator_size() const
{
	return sizeof(size_t) + sizeof(logger*) + sizeof(memory*) + sizeof(memory::allocate_mode) + sizeof(void*);
}

unsigned char buddy_system::compact_block_size_and_block_occup(size_t size_as_power_of_2, bool block_occup) const
{
	return (size_as_power_of_2 << 1) | (block_occup & 1);
}

size_t buddy_system::get_block_avail_size(void* target_ptr) const
{

	return 1 << (*reinterpret_cast<unsigned char*>(target_ptr) >> 1);
}

size_t buddy_system::get_occup_block_serv_size() const
{
	return sizeof(unsigned char);
}

logger* buddy_system::get_logger() const noexcept
{
	return *reinterpret_cast<logger**>(reinterpret_cast<unsigned char*>(_all_memory) + sizeof(size_t));
}

memory* buddy_system::get_memory() const
{
...
std::string buddy_system::get_allocate_mode_string(allocate_mode method) const
{
...
std::string buddy_system::get_pointer_address(void* target_ptr) const
{
...
void buddy_system::debug_alloc(const void* target_ptr) const
{
...
void* buddy_system::get_buddies(void* target_ptr) const
{
...
bool buddy_system::is_free_block(void* target_ptr) const
{
	return *reinterpret_cast<unsigned char*>(target_ptr) & 1;
}

void buddy_system::dump_allocator_state(bool is_allocate) const noexcept
{
	{
		auto* log = get_logger();
		if (log == nullptr)
...
size_t buddy_system::get_avail_block_serv_size() const
{
	return sizeof(unsigned char) + 2 * sizeof(void*);
}

void* buddy_system::allocate(size_t requested_block_size) const
{
...
void buddy_system::deallocate(void const* const block_to_deallocate_address) const
{

	auto curr_block = reinterpret_cast<void*>(reinterpret_cast<unsigned char*>(const_cast <void*>(block_to_deallocate_address)) - 1);
...
buddy_system::buddy_system(logger* log, memory* allocator, size_t size_as_power_of_2, memory::allocate_mode method)
{
