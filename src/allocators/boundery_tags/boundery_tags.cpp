#include "boundery_tags.h"
#include <string>

size_t boundery_tags::get_service_part_allocator_size() const
{
	return sizeof(size_t) + sizeof(logger*) + sizeof(memory::allocate_mode) + sizeof(memory*) + sizeof(void*);
}

boundery_tags::boundery_tags(logger* log,
	memory* allocator,
	size_t size,
...
std::string boundery_tags::get_allocate_mode_string(allocate_mode method) const
{
...
std::string boundery_tags::get_address(void* target_ptr) const
{
...
void boundery_tags::debug_alloc(const void* target_ptr) const
{
	...
	std::string add = get_address(const_cast<void*>(target_ptr));
	std::string massege = "Block at address " + add + " state before deallocation:\n[ " + buff + "]";
	log->log(massege, logger::severity::debug);
}

void boundery_tags::dump_allocator_state(bool is_allocate) const noexcept
{
	auto* log = get_logger();
	if (log == nullptr)
	{
...
void* boundery_tags::allocate(size_t requested_block_size) const
{
...
void boundery_tags::deallocate(void const* const block_to_deallocate_address) const
{
	auto true_target_to_dealloc = reinterpret_cast<void*>(reinterpret_cast<unsigned char*>(const_cast<void*>(block_to_deallocate_address)) - get_occup_block_serv_size());
